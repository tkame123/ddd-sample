...
type callOrderCancelSagaWhenTicketCancellationRejectedHandler struct {
...
func NewCallOrderCancelSagaWhenTicketCancellationRejectedHandler(
	rep repository.Repository,
	orderSVC service.OrderService,
	kitchenAPI external_service.KitchenAPI,
	billingAPI external_service.BillingAPI,
) domain_event.EventHandler {
	return &callOrderCancelSagaWhenTicketCancellationRejectedHandler{
		rep:        rep,
		orderSVC:   orderSVC,
...
func (h *callOrderCancelSagaWhenTicketCancellationRejectedHandler) Handler(ctx context.Context, mes *message.Message) error {
	if mes.Subject.Type != message.Type_TYPE_EVENT_TICKET_CANCELLATION_REJECTED {
