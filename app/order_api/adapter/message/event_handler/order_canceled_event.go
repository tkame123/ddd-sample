...
type callOrderCancelSagaWhenOrderCanceledHandler struct {
...
func NewCallOrderCancelSagaWhenOrderCanceledHandler(
	rep repository.Repository,
	orderSVC service.OrderService,
	kitchenAPI external_service.KitchenAPI,
	billingAPI external_service.BillingAPI,
) domain_event.EventHandler {
	return &callOrderCancelSagaWhenOrderCanceledHandler{
		rep:        rep,
		orderSVC:   orderSVC,
...
func (h *callOrderCancelSagaWhenOrderCanceledHandler) Handler(ctx context.Context, mes *message.Message) error {
	if mes.Subject.Type != message.Type_TYPE_EVENT_ORDER_CANCELED {
