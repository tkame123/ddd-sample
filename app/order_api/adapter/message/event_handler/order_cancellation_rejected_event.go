...
type callOrderCancelSagaWhenOrderCancellationRejectedHandler struct {
...
func NewCallOrderCancelSagaWhenOrderCancellationRejectedHandler(
	rep repository.Repository,
	orderSVC service.OrderService,
	kitchenAPI external_service.KitchenAPI,
	billingAPI external_service.BillingAPI,
) domain_event.EventHandler {
	return &callOrderCancelSagaWhenOrderCancellationRejectedHandler{
		rep:        rep,
		orderSVC:   orderSVC,
...
func (h *callOrderCancelSagaWhenOrderCancellationRejectedHandler) Handler(ctx context.Context, mes *message.Message) error {
	if mes.Subject.Type != message.Type_TYPE_EVENT_ORDER_CANCELLATION_REJECTED {
