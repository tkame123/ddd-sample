...
type callOrderCancelSagaWhenOrderCancellationConfirmedHandler struct {
...
func NewCallOrderCancelSagaWhenOrderCancellationConfirmedHandler(
	rep repository.Repository,
	orderSVC service.OrderService,
	kitchenAPI external_service.KitchenAPI,
	billingAPI external_service.BillingAPI,
) domain_event.EventHandler {
	return &callOrderCancelSagaWhenOrderCancellationConfirmedHandler{
		rep:        rep,
		orderSVC:   orderSVC,
...
func (h *callOrderCancelSagaWhenOrderCancellationConfirmedHandler) Handler(ctx context.Context, mes *message.Message) error {
	if mes.Subject.Type != message.Type_TYPE_EVENT_ORDER_CANCELLATION_CONFIRMED {
